...
mod tests {
    use super::{
        EnrichedMessage, EnrichmentMetadata, IncomingMessage, MediaKind, MessageSource,
        ProcessedMessage, ProcessingCompleteness, RetryableMessage, SourceMetadata,
    };
    ...
    fn processed(incomplete: ProcessingCompleteness) -> ProcessedMessage {
        ProcessedMessage {
            enriched: EnrichedMessage {
                original: sample_incoming(),
                urls: Vec::new(),
                url_contents: Vec::new(),
            },
            llm_response: None,
            incomplete,
            fallback_source_urls: Vec::new(),
            fallback_tool_results: Vec::new(),
            fallback_title: None,
    ...
    #[test]
    fn retryable_from_incoming_copies_fields() {
        let r = RetryableMessage::from(&sample_incoming());
        assert_eq!(r.text, "hi");
        assert!(r.attachments.is_empty());
    }

    #[test]
    fn processed_message_incompleteness() {
        assert!(!processed(ProcessingCompleteness::Complete).is_incomplete());
        assert!(processed(ProcessingCompleteness::IncompleteVisionUnavailable).is_incomplete());
    }

    #[test]
    fn incoming_message_debug_names_struct() {
        assert!(format!("{:?}", sample_incoming()).contains("IncomingMessage"));
    }
